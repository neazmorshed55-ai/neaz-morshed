-- ==========================================
-- Resume Management Tables for Supabase (Fixed Version)
-- This version drops and recreates everything cleanly
-- ==========================================

-- Drop existing policies first
DROP POLICY IF EXISTS "Allow public read access" ON resume_settings;
DROP POLICY IF EXISTS "Allow authenticated users to insert" ON resume_settings;
DROP POLICY IF EXISTS "Allow authenticated users to update" ON resume_settings;
DROP POLICY IF EXISTS "Allow authenticated users to delete" ON resume_settings;

DROP POLICY IF EXISTS "Allow public read access" ON resume_stats;
DROP POLICY IF EXISTS "Allow authenticated users to insert" ON resume_stats;
DROP POLICY IF EXISTS "Allow authenticated users to update" ON resume_stats;
DROP POLICY IF EXISTS "Allow authenticated users to delete" ON resume_stats;

DROP POLICY IF EXISTS "Allow public read access" ON resume_skills;
DROP POLICY IF EXISTS "Allow authenticated users to insert" ON resume_skills;
DROP POLICY IF EXISTS "Allow authenticated users to update" ON resume_skills;
DROP POLICY IF EXISTS "Allow authenticated users to delete" ON resume_skills;

DROP POLICY IF EXISTS "Allow public read access" ON resume_experiences;
DROP POLICY IF EXISTS "Allow authenticated users to insert" ON resume_experiences;
DROP POLICY IF EXISTS "Allow authenticated users to update" ON resume_experiences;
DROP POLICY IF EXISTS "Allow authenticated users to delete" ON resume_experiences;

DROP POLICY IF EXISTS "Allow public read access" ON resume_education;
DROP POLICY IF EXISTS "Allow authenticated users to insert" ON resume_education;
DROP POLICY IF EXISTS "Allow authenticated users to update" ON resume_education;
DROP POLICY IF EXISTS "Allow authenticated users to delete" ON resume_education;

DROP POLICY IF EXISTS "Allow public read access" ON resume_certifications;
DROP POLICY IF EXISTS "Allow authenticated users to insert" ON resume_certifications;
DROP POLICY IF EXISTS "Allow authenticated users to update" ON resume_certifications;
DROP POLICY IF EXISTS "Allow authenticated users to delete" ON resume_certifications;

DROP POLICY IF EXISTS "Allow public read access" ON resume_capstone;
DROP POLICY IF EXISTS "Allow authenticated users to insert" ON resume_capstone;
DROP POLICY IF EXISTS "Allow authenticated users to update" ON resume_capstone;
DROP POLICY IF EXISTS "Allow authenticated users to delete" ON resume_capstone;
...
INSERT INTO resume_education (institution, degree, field, location, start_date, end_date, order_index)
SELECT * FROM (VALUES
...
) AS v(institution, degree, field, location, start_date, end_date, order_index)
WHERE NOT EXISTS (SELECT 1 FROM resume_education);

-- Insert default certifications
INSERT INTO resume_certifications (title, issuer, date, expiry, credential_url, order_index)
SELECT * FROM (VALUES
...
) AS v(title, issuer, date, expiry, credential_url, order_index)
...
DROP TRIGGER IF EXISTS update_resume_settings_updated_at ON resume_settings;
DROP TRIGGER IF EXISTS update_resume_stats_updated_at ON resume_stats;
DROP TRIGGER IF EXISTS update_resume_skills_updated_at ON resume_skills;
DROP TRIGGER IF EXISTS update_resume_experiences_updated_at ON resume_experiences;
...
DROP TRIGGER IF EXISTS update_resume_capstone_updated_at ON resume_capstone;
