...
function StructureManager({ onClose, categories: initialCategories, subSkills: initialSubSkills }: { onClose: () => void, categories: SkillCategory[], subSkills: SubSkill[] }) {
  ...
  const [skillForm, setSkillForm] = useState({ title: '', category_id: '', is_active: true, order_index: 0 });
  ...
  const handleSaveSkill = async () => {
    ...
    try {
      ...
      await refreshData();
      setSkillForm({ ...skillForm, title: '', order_index: 0 }); // keep category selected
      setEditingId(null);
    } catch (e) {
      console.error(e);
            ...
            <div className="space-y-8">
              ...
              <div className="bg-slate-800/50 p-6 rounded-xl border border-white/5">
                ...
                <div className="flex gap-4 items-center">
                  ...
                  {editingId && (
                    <button onClick={() => { setEditingId(null); setSkillForm({ ...skillForm, title: '', is_active: true }); }} className="bg-slate-700 text-white font-bold px-4 py-3 rounded-lg mt-4"> Cancel </button>
                  )}
                </div>
              </div>
                      ...
                      <div className="space-y-2 pl-4 border-l-2 border-[#2ecc71]/20">
                        {catSkills.map(skill => (
                          <div key={skill.id} className="flex items-center justify-between bg-slate-800/30 p-3 rounded-lg border border-white/5">
                            ...
                            <div className="flex gap-2">
                              <button onClick={() => { setEditingId(skill.id); setSkillForm({ title: skill.title, category_id: skill.category_id, is_active: Boolean(skill.is_active), order_index: skill.order_index }); }} className="p-1.5 text-[#2ecc71] hover:bg-[#2ecc71]/10 rounded"><Edit2 size={14} /></button>
